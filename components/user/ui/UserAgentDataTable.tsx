        ...
        cell: (info) => {
            ...
            return (
                <div className="flex items-center">
                    <div className="w-10 h-10 rounded-[3px] bg-cover bg-center" style={{ backgroundImage: `url(${agent.avatar})` }} />
...
const UserAgentDataTable = ({ username, agents }: UserAgentDataTableProps) => {
    ...
    return (
        <Fragment>
            <div className="flex items-center justify-between mb-12">
                <div className="text-white text-xl sm:text-3xl font-normal font-neopixel tracking-tight uppercase">
                    <span className="text-blue-200">{username}</span> AGENTS
                </div>
                <div>&nbps;</div>
            </div>
                        ...
                        {table.getHeaderGroups().map((headerGroup) => (
                            <tr key={headerGroup.id}>
                                {headerGroup.headers.map((header, index) => (
                                    <th key={index} className={`px-2.5 border-none ${index === 0 ? 'sticky left-0 bg-black sm:bg-transparent z-10 w-1/3' : ''}`}>
                                            ...
                                            <tbody>
                                                <tr>
                                                    {row.getVisibleCells().map((cell, index) => (
                                                        <td key={index} className={`py-1 px-2.5 ${index === 0 ? 'sticky left-0 bg-black sm:bg-transparent z-10 w-1/3' : ''}`}>
                                                    ...
                                                    <td colSpan={6} className="py-1 px-2.5">
                                                        ...
                                                        <div className="flex items-center justify-between my-5">
                                                            <div>
                                                                ...
                                                                <div className="text-slate-400 text-xs font-semibold font-figtree tracking-tight">Inactive</div>
                                                            </div>
...
export default UserAgentDataTable;
